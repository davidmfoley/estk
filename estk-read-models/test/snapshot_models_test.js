...
describe('snapshot models', () => {
  let model, storage, snapshotExample;
  ...
  describe('getting current state', () => {
    describe('when not in cache, ', () => {
      ...
      beforeEach(() => {
        storage.get.resolves({notFound: true});
        model.get.resolves({ state: { id: '42', name: 'arthur' }, bookmark: {} });
    ...
    describe('when present in cache, ', () => {
      beforeEach(() => {
        storage.get.resolves({bookmark: {}, state: { id: '42', name: 'arthur' }});
        model.get.throws(new Error());
        model.update.resolves({ state: { id: '42', name: 'arthur', hungry: true}, bookmark: {}});
      });
