...
export default function buildEventQuery(filter: any, bookmark: EventStreamBookmark | null | undefined, chunkSize: number = 2500): DatabaseQuery {
...
function buildTargetTypeClause(targetType: string, spec: any, offset: number): QueryPiece {
  if (spec === '*') {
    return {
  ...
  if (spec.action) {
    ...
    const actionParams = actions.map(
      (a: string, i: number) => `$${offset + i + 2}`
    ).join(', ');
